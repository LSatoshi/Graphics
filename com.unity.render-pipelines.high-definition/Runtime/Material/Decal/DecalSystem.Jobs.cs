...
namespace UnityEngine.Rendering.HighDefinition
{
    partial class DecalSystem
    {
        ...
        partial class DecalSet
        {
            ...
            {
                ArrayUtils.ResizeTransformArray(ref m_CachedTransforms, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_Positions, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_Rotations, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_Scales, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_Sizes, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_Offsets, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_ResolvedRotations, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_ResolvedScales, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_ResolvedSizeOffsets, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_ScaleModes, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_NormalToWorlds, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_DecalToWorlds, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_BoundingSpheres, newCapacity);
                ArrayUtils.ResizeNativeArray(ref m_Dirty, newCapacity);
                ArrayUtils.ResizeArray(ref m_CachedBoundingSpheres, newCapacity);
            }
            ...
            internal void StartUpdateJob()
            {
                ...
                var updateJob = new UpdateJob()
                {
                    ...
                    resolvedSizedOffsets = m_ResolvedSizeOffsets,
                    dirty = m_Dirty,
                    rawSizes = m_Sizes,
        ...
        internal struct UpdateJob : IJobParallelForTransform
        {
            ...
            public NativeArray<float4x4> resolvedSizedOffsets;
            ...
            public void Execute(int index, TransformAccess transform)
            {
                ...
                if (isDirty || rotationChanged || scaleChanged)
                    resolvedSizedOffsets[index] = math.mul(float4x4.Translate(resolveDecalOffset(index, resolvedScales[index], transform)), float4x4.Scale(resolveDecalSize(index, resolvedScales[index], transform)));
                ...
                float4x4 sizeOffset = resolvedSizedOffsets[index];
